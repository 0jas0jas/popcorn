...
const genreDict: GenreDict = {
  ...
  function ReturnMovies({ movieList }: { movieList: string[] }) {
    ...
    return (
        <div className="min-h-screen col-span-2 grid grid-cols-2 gap-12 p-24 place-items-center font-mono bg-gray-900">
            {movies.filter(movie => movie).map(movieData => {
                if (!movieData.results[0]) return null;
                let genre = "";
                movieData.results[0].genre_ids.forEach((id: number) => {
                    genre = (genreDict[id] ? genreDict[id] : "Unknown") + " | " + genre;
                });
                return (
